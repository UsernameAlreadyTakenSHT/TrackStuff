package io.github.usernamealreadytakensht.trackstuff.data.repository

import android.util.Log
import io.github.usernamealreadytakensht.trackstuff.data.imdb.ImdbRepository
import io.github.usernamealreadytakensht.trackstuff.data.omdborg.OmdbOrgRepository
import io.github.usernamealreadytakensht.trackstuff.data.remote.omdb.OmdbApi
import io.github.usernamealreadytakensht.trackstuff.data.remote.omdb.OmdbTitle
import io.github.usernamealreadytakensht.trackstuff.data.remote.tmdb.TmdbApi
import io.github.usernamealreadytakensht.trackstuff.data.remote.tmdb.TmdbCredits
...
import io.github.usernamealreadytakensht.trackstuff.data.remote.tvdb.TvdbApi
import io.github.usernamealreadytakensht.trackstuff.data.remote.tvdb.TvdbExtended
import io.github.usernamealreadytakensht.trackstuff.data.remote.tvdb.TvdbLoginRequest
import io.github.usernamealreadytakensht.trackstuff.data.remote.tvdb.TvdbSearchItem
import io.github.usernamealreadytakensht.trackstuff.data.settings.AppSettings
...
import io.github.usernamealreadytakensht.trackstuff.domain.WatchProvider
...
import io.github.usernamealreadytakensht.trackstuff.domain.fillMissingFrom
...
private fun errMsg(e: Exception): String = io.github.usernamealreadytakensht.trackstuff.data.remote.describeError(e)
...
class MetadataRepository(
    ...
    private suspend fun discoverFingerprint(s: io.github.usernamealreadytakensht.trackstuff.data.settings.AppSettings): String =
    ...
    suspend fun discover(force: Boolean = false): DiscoverOutcome {
        ...
        if (force && io.github.usernamealreadytakensht.trackstuff.data.remote.RefreshLimiter.tryAcquire("discover")) io.github.usernamealreadytakensht.trackstuff.data.remote.Network.evict { url -> LIST_URL_MARKERS.any { it in url } }
    ...
    private val store = object {
        private val adapter by lazy { io.github.usernamealreadytakensht.trackstuff.data.remote.Network.moshi.adapter(DiscoverOutcome::class.java) }
    ...
    private suspend fun discoverNow(): DiscoverOutcome {
        ...
        try {
            ...
            if (api == null) problems += "TVDB: API key not set"
                ...
                suspend fun tvdbRow(category: String, title: String, media: SectionMedia, call: suspend () -> List<io.github.usernamealreadytakensht.trackstuff.data.remote.tvdb.TvdbBaseRecord>?) {
    ...
    private fun io.github.usernamealreadytakensht.trackstuff.data.remote.tvdb.TvdbBaseRecord.toSummary(isSeries: Boolean) = MediaSummary(
    ...
    suspend fun resolveLink(url: String): MediaSummary? {
        ...
        LINK_TRAKT.find(url)?.let { m ->
            ...
            val api = io.github.usernamealreadytakensht.trackstuff.data.remote.trakt.TraktApi.create(s.traktClientId) { null }
    ...
    suspend fun watchProviders(ids: ExternalIds, isSeries: Boolean): WatchProviders? {
        ...
        fun List<io.github.usernamealreadytakensht.trackstuff.data.remote.tmdb.TmdbProvider>.toDomain() = sortedBy { it.priority }.map { WatchProvider(it.name, it.logoPath?.let { p -> TmdbApi.IMAGE_BASE + "w92" + p }) }
    ...
    suspend fun details(initialIds: ExternalIds, isSeries: Boolean, fallbackTitle: String?, fallbackYear: Int?): MediaDetails {
        ...
        return result.copy(
            ...
            ratingsSource = if (externalRatings?.hasExternal == true) DataSource.OMDB else result.ratingsSource,
            creditsSource = if (result.credits.isEmpty && externalCredits?.isEmpty == false) DataSource.OMDB else result.creditsSource,
    ...
    private suspend fun urlAlive(url: String): Boolean = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
        runCatching {
            io.github.usernamealreadytakensht.trackstuff.data.remote.Network.client.newCall(okhttp3.Request.Builder().url(url).head().build()).execute().use { it.code != 404 }
        }.getOrDefault(true)
    }
    ...
    private fun TvdbExtended.toDetails(isSeries: Boolean, language: String): MediaDetails {
        ...
        return MediaDetails(
            ...
            credits = Credits(
                ...
                cast = characters.filter { it.peopleType == "Actor" }.sortedWith(compareByDescending<io.github.usernamealreadytakensht.trackstuff.data.remote.tvdb.TvdbCharacter> { it.isFeatured == true }.thenBy { it.sort ?: 99 })
    ...
    private fun OmdbTitle.toDetails(isSeries: Boolean): MediaDetails {
        ...
        return MediaDetails(
            ...
            ratingsSource = DataSource.OMDB, creditsSource = DataSource.OMDB,
