...
import io.github.usernamealreadytakensht.trackstuff.data.local.OmdbAliasEntity
import io.github.usernamealreadytakensht.trackstuff.data.local.OmdbCastEntity
import io.github.usernamealreadytakensht.trackstuff.data.local.OmdbOrgDao
import io.github.usernamealreadytakensht.trackstuff.data.local.OmdbTitleEntity
...
class OmdbOrgRepository(
    ...
    suspend fun search(query: String, language: String): List<MediaSummary> {
        ...
        return dao.searchFts(q, match, io.github.usernamealreadytakensht.trackstuff.data.imdb.ImdbRepository.SEARCH_LIMIT).map { it.toSummary(lang) }
    }
    ...
    suspend fun details(e: OmdbTitleEntity, language: String): MediaDetails {
        ...
        return MediaDetails(
            ...
            countries = e.countries.split(',').filter { it.isNotBlank() }.map { io.github.usernamealreadytakensht.trackstuff.data.repository.MetadataRepository.countryName(it) },
