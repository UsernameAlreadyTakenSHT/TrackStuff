package io.github.usernamealreadytakensht.trackstuff.ui.search
...
import io.github.usernamealreadytakensht.trackstuff.ui.components.MediaCard
    ...
    Scaffold(topBar = { TopAppBar(title = { Text(stringResource(R.string.search_title)) }) }) { padding ->
        Column(Modifier.padding(padding).fillMaxSize()) {
            ...
            Row(Modifier.horizontalScroll(rememberScrollState()).padding(horizontal = 16.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                MetadataRepository.SEARCH_ORDER.filter { it != io.github.usernamealreadytakensht.trackstuff.domain.DataSource.OMDB }.forEach { s -> FilterChip(selected = state.only == s, onClick = { vm.setSource(s) }, label = { Text(s.label) }) }
            }
            ...
            if (state.problems.isNotEmpty() && (state.results.isEmpty() || state.source != io.github.usernamealreadytakensht.trackstuff.domain.DataSource.TMDB)) {
                Text(
                ...
                state.results.isEmpty() && !state.searched -> Box(Modifier.fillMaxSize().padding(32.dp), contentAlignment = Alignment.Center) {
                    ...
                    items(state.results) { r ->
                        ...
                        MediaCard(
                            ...
                            kind = r.kindHint ?: if (r.isSeries) io.github.usernamealreadytakensht.trackstuff.domain.MediaKind.SERIES else io.github.usernamealreadytakensht.trackstuff.domain.MediaKind.MOVIE,
                            status = null,
