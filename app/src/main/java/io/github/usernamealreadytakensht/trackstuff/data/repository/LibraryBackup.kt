...
class LibraryBackup(private val dao: MediaDao, private val library: LibraryRepository) {
    ...
    private class LegacyStatusAdapter {
        @com.squareup.moshi.FromJson fun fromJson(name: String): io.github.usernamealreadytakensht.trackstuff.domain.WatchStatus = io.github.usernamealreadytakensht.trackstuff.data.local.MediaConverters.legacyStatus(name)
        @com.squareup.moshi.ToJson fun toJson(s: io.github.usernamealreadytakensht.trackstuff.domain.WatchStatus): String = s.name
    }

    private val adapter = Moshi.Builder()
        .add(LegacyStatusAdapter())
        .add(io.github.usernamealreadytakensht.trackstuff.data.remote.NullToEmptyListAdapterFactory())
